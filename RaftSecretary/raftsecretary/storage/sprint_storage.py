...
def save_sprint_entries(
    db_path: Path,
    category_key: str,
    entries: list[SprintEntry],
) -> None:
    with sqlite3.connect(db_path) as connection:
        _ensure_sprint_schema(connection)
        connection.execute(
            "DELETE FROM sprint_results WHERE category_key = ?",
            (category_key,),
        )
...
def load_sprint_entries(db_path: Path, category_key: str) -> list[SprintEntry]:
    with sqlite3.connect(db_path) as connection:
        _ensure_sprint_schema(connection)
...
def save_sprint_lineup_flags(
    ...
    with sqlite3.connect(db_path) as connection:
        _ensure_sprint_schema(connection)
        connection.execute(
            "DELETE FROM sprint_lineup_flags WHERE category_key = ?",
            (category_key,),
        )
        rows = []
        ...
        connection.executemany(
            """
            INSERT INTO sprint_lineup_flags (
                category_key,
                team_name,
                member_order,
                is_active
...
def _ensure_sprint_schema(connection: sqlite3.Connection) -> None:
    connection.execute(
        """
    ...
    columns = {
        row[1]
        for row in connection.execute("PRAGMA table_info(sprint_results)").fetchall()
    }
    if "start_time" not in columns:
        connection.execute(
            "ALTER TABLE sprint_results ADD COLUMN start_time TEXT NOT NULL DEFAULT ''"
        )
